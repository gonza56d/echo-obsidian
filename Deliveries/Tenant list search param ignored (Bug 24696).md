...
## PRs
- [#2206](https://github.com/taller-projects/echo-backend/pull/2206) → dev — OPEN 2026-09-02 (`23a40dc2`, branch `24696/tenant_search_filter`)
...
- Shared filter → also fixes `/internal/tenants` (`app/modules/tenant/internal_routers.py`).
- Tests: `tests/unit/test_tenant_admin.py::TestTenantListSearch` (module-scoped tenant "Zyxq Search Target"; positive narrows + excludes other fixture tenants, negative returns empty). Both verified to FAIL on `origin/dev` without the fix.

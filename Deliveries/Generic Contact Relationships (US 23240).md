...
- [#1657](https://github.com/taller-projects/echo-backend/pull/1657) **M2+M3-additive** ([23242](https://dev.azure.com/TallerInternTools/Echo%20Core/_workitems/edit/23242)/[23243](https://dev.azure.com/TallerInternTools/Echo%20Core/_workitems/edit/23243), 2026-06-26) — label-aware `_relationship_is_active_expr` (Client rule: interaction ≤12mo OR live end_date, AND account gate); per-rel `state` + contact `relationship_state` exposed; `relationship_state__in` filter.
- [#1669](https://github.com/taller-projects/echo-backend/pull/1669) review nits ([Task 23292](https://dev.azure.com/TallerInternTools/Echo%20Core/_workitems/edit/23292), 2026-06-29) — internal get-by-id projection parity; CI-runnable unit tests (CI = unit only!).
- [#1670](https://github.com/taller-projects/echo-backend/pull/1670) **column DROP** ([Task 23264](https://dev.azure.com/TallerInternTools/Echo%20Core/_workitems/edit/23264), 2026-06-29) — migration `mx7qkw9n2r4v` drops column+trigger+function; real migration round-trip test **caught a deploy-breaking bug** (view column type text vs varchar → `CAST(... AS VARCHAR)`).
...
## Gotchas (reusable)
- `view_smart_search_contacts_info` + `mv_smart_search_*` are **owner-managed out-of-band** ([ticket 19757](https://dev.azure.com/TallerInternTools/Echo%20Core/_workitems/edit/19757)): deployed = 40 cols, repo SQL = 12 cols. Any `CREATE OR REPLACE VIEW` on them fails "cannot drop columns from view" → always guard with a shape check (`_view_is_repo_shaped`).
...
## Pending
- **QA/PROD still gated**: out-of-band matviews must be recreated to compute the value inline BEFORE `mx7qkw9n2r4v` runs there (dev done; coordinate with view owner, ref [19757](https://dev.azure.com/TallerInternTools/Echo%20Core/_workitems/edit/19757)).
- Dashboard counts reshape (#4) + `alumni_*` naming removal (#5, FE [US 23259](https://dev.azure.com/TallerInternTools/Echo%20Core/_workitems/edit/23259)) + M3 perf measurement — still open under [US 23240](https://dev.azure.com/TallerInternTools/Echo%20Core/_workitems/edit/23240).

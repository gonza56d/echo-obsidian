...
- `FutureInteractionService._resolve_roles` (typed `FutureInteractionFilter`, as are `list_for_viewer` / `list_internal` since `a3cace05`): calls `_restrict_entity_types(entity_filter, {candidate})`, which folds a singular `linked_entity_type` into the plural form and narrows `linked_entity_type__in` to candidates (short-circuits to an empty page when no candidate type survives), then asks `ApplicationService.get_talent_ids_applied_to_roles(role_ids, bound)` (new, explicit tenant predicate, `DISTINCT talent_id`, predicate `status IS NOT NULL OR workflow_step_id IS NOT NULL`) and keeps `bound ∩ answer`. `bound` = whatever an earlier step put in `linked_entity_id__in`, else the candidate refs of touchpoints (`repo.get_person_refs`).
...
## Team review (Pedro, 2026-09-25)
- [Review 5320694250](https://github.com/taller-projects/echo-backend/pull/2350#pullrequestreview-5320694250) → **APPROVED, READY WITH NITS** (arch 15/0, PRD 21/21 BE, T&S 13/0, CI green). Pedro confirmed the non-Matched predicate reading over the PRD's literal "status NULL" wording and will fix the PRD/US himself (Matched = status NULL **and** no step; comma-separated wire format; blank param = no filter).
- All 7 nits fixed in `a3cace05`: `_resolve_roles` reuses `_restrict_entity_types` (identical result for None / supplied / singular); role block moved above the comment explaining `_restrict_entity_types`; `list_for_viewer` / `list_internal` typed `FutureInteractionFilter`; `get_talent_ids_applied_to_roles` docstring states the contract (no consumer name); internal router `description=` split; stray blank line removed; 2 new internal tests (caller-supplied `linked_entity_id__in` contact + applicant with `role_id__in` → only the applicant; plural `linked_entity_type__in=candidate,contact` + role). Nit 6 (role ids not checked against `visible_organizations`) **accepted and documented** on `_resolve_roles` and in the PR body rather than adding a `RoleService` check: the answer only narrows already-visible people. 143 touched-file tests green; PR body updated (24 new tests).
- Follow-ups Pedro listed outside the PR (unticketed): centralize the non-Matched predicate (5 copies, e.g. `Application.is_not_matched()`); internal plural `owner_user_id__in` bound is tenant-wide (correct, negligible cost).

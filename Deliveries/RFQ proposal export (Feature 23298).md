...
- **#2179 (empty sections fix)**: self-review fixes pushed `f78b2cad` 2026-08-28 → CI + peer review → merge to dev → promote qa/main with the next batch. Unfiled follow-ups: proposal-path all-null contact (hoist `_contact_or_none` to a shared export helper if ever needed); RFQ template `{% extends %}` refactor.

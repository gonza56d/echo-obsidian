...
- Suite: full unit + multitenancy green, **3701 passed, 1 xpassed** (fresh testcontainer upgrade exercises the migration; the 5 flaky adoption tests didn't fire this run).
...
- **Courtesy heads-up BEFORE merge**: identify the Taller prod reader script owner (user id + psql query in Gotchas; the lookup was classifier-blocked in-session) and tell them the table they read froze on 2026-02.
- **Pre-prod-promotion check**: run the `pg_depend` dependent-objects query against prod (dev verified clean; query saved in the PR body).

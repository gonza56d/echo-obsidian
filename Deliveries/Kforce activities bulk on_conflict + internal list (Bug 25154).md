...
- `/pr-review` **r2** (2026-09-25, on `5e201d7b`): **READY WITH NITS**, 0 blockers, 0 regressions (arch 15/0/1, tests 13/0/3 with 28 passed observed, tickets 18/18). All 8 r1 nits verified FIXED. r2 nits fixed in `ed8addf9` + dev merged in `4d6b9e89` (both pushed from worktree `25154-r2-nits`, branch `25154/review-nits-r2` → `HEAD:25154/activities-bulk-on-conflict-internal-list`); PR body rewritten via `gh api` PATCH with the post-tie-break EXPLAIN table. 63 tests green on the merged tree.
...
- **Review follow-up r2 `ed8addf9`:** an empty `kforce_external_id__in=` no longer counts as a selective filter (`not self.kforce_external_id__in` in the `model_validator` → 422 like no filter at all; before it answered an always-empty page); route description says "non-empty". Bulk POST test pins `body[0]["id"]` == the inserted row's Echo id; route tests add `kind__in` and the cross-tenant negative for `relationship_id`; `test_contact_kforce_external_id_filter` imports the cap from `contact/constants.py`; trimmed the `bulk_create` comment and the `tenant_query` docstring (no plan shape in code). Tests 28 → 31 across both files. `4d6b9e89` merges dev (#2346 touched `contact/filters.py`, auto-merged clean).
...
- **Empty id list is a 422, not an empty page** (review r2): `?kforce_external_id__in=` parses to `[]` through `FilterDepends`, which passed the `is None` gate and emitted `IN (NULL) AND 1 != 1`. Harmless, but a caller that sends an empty batch has a bug and should hear it.
...
- kforce-dev `EXPLAIN (ANALYZE, BUFFERS)` **after the `id` tie-break** (2026-09-25, 1.50M `contact_activity`): 100 ids + `kind` → unique-index scan → nested loop → index-only `uq_contact_id_tenant_id` → quicksort 50 kB, 111 ms cold / 1.4 ms warm, count 1.5 ms; heaviest `contact_id` (12,441 activities, `size=30000`) → bitmap on `contact_activity_contact_id_idx` → quicksort 3.3 MB, 30 / 15 ms, count 4.4 ms; heaviest `relationship_id` (12,441) → bitmap on `contact_activity_relationship_id_idx` → nested loop → parallel quicksort 2 MB, 46 / 42 ms, count 27 ms. All index-served; the sort only covers matched rows.
- Sampling the heaviest contact / relationship on kforce-dev is a full `GROUP BY` over 1.5M rows (a few seconds each); fine ad hoc, do not put it in a test.
- In a worktree session a pytest path that does not exist makes pytest exit 4 with `N warnings in 0.00s` and NO `passed` line; behind a `| tail` the `&&` chain still continues (zsh has no pipefail) — check for the word `passed` before trusting a push gated on tests.

## Pending
- PR review + merge → Bug 25154 / Task 25155 → Closed; dev deploy; tell Emi (P2-4 in DEV unblocks the activities push; P2-5 route + caveat; **a kind-only or empty-id call answers 422** — always send ids, `contact_id` or `relationship_id`).
- Task 25155 wording says "filterable by kind and kforce_external_id__in"; add a comment there that a selective filter is mandatory (review r2 question).
- Review r2 out-of-scope: `TenantContextError` is a bare `Exception` → catch-all 500 (unreachable on `/internal`; same in `list_trackers`); mapping it to an `EchoException` is a global decision. `_api_key` test helper duplicated in ~20 files.
- Bug 25111 and Task 25112 were still **In revision** on 2026-09-25 although #2344 / #2345 merged → close them.
- qa/main promotion.
- **Out-of-scope follow-ups from review r1:** activities bulk POST never checks that the path contact belongs to the request tenant nor that the relationship belongs to the contact (cross-tenant write + refresh; `bulk_update` gates via `ContactService.get_by_id`) — filed as [Bug 25158](https://dev.azure.com/TallerInternTools/Echo%20Core/_workitems/edit/25158) → **fix PR [#2351](https://github.com/taller-projects/echo-backend/pull/2351) stacked on #2349, see [[Activities bulk tenant ownership gate (Bug 25158)]]** (Severity 2, Sprint 45, parent 24972; also covers unchecked body `consultant_id`/`recruiter_id`/`account_manager_id` on POST + PATCH, whose nested response leaks another tenant's user email); `order_by=consultant|recruiter|account_manager|organization` → 500 on public and internal activity lists (allow-list sort fields); `ActivityBulkCreate.activities` has no batch cap (PATCH caps 500); the per-relationship activity mount is `deprecated=` although the push depends on its POST/PATCH; `AdminParams`/`AdminPage` duplicate `app/core/pagination.py`.

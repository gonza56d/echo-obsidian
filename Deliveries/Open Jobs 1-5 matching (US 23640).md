...
**Update — batched release with US 22248 (same day, this session).** Decision changed: instead of shipping open-jobs alone (with `ehjnwsqitqve` re-pointed onto the release head `q8vd3mzk7w2p`), the two features now promote to qa/main **together**. The interview-assessment backfill `3gufg5ykw01g` (US 22248, [[Interview assessment change history (US 22248)]]) ships in the same batch via its own cherry-pick PRs ([#1879](https://github.com/taller-projects/echo-backend/pull/1879) → qa, [#1880](https://github.com/taller-projects/echo-backend/pull/1880) → main). So `ehjnwsqitqve`'s `down_revision` was re-pointed **`q8vd3mzk7w2p` → `3gufg5ykw01g`** on both #1875/#1876 (commits: qa `ef422dac`, main `5191ab2f`). `51r81g9s8arp` (contact index) stays skipped. **Final chain on each release branch: `q8vd3mzk7w2p → 3gufg5ykw01g → ehjnwsqitqve`** (single head, verified `alembic heads` on the combined state). qa DB **and** PROD DB both confirmed at `q8vd3mzk7w2p` (read-only). **⚠️ Merge order per env: interview PR first (#1879/#1880), then open-jobs (#1875/#1876).** This dissolves the "future down_revision conflict" gotcha above — the wipe no longer diverges from a `51r81g9s8arp` parent, and `3gufg5ykw01g` is now on the release branch. PR titles/descriptions updated to reflect the new chain + merge order.

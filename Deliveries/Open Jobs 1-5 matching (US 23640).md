...
**Update — combined with US 22248 into this PR (same day, this session).** The two features release together **in one PR per environment** (not separate PRs): the interview backfill `3gufg5ykw01g` + its 5 commits (US 22248, [[Interview assessment change history (US 22248)]]) were cherry-picked directly onto the #1875/#1876 branches (**fast-forward, no force-push**). Each PR now carries BOTH features and a self-contained chain `q8vd3mzk7w2p → 3gufg5ykw01g → ehjnwsqitqve` (single head, verified `alembic heads`). `ehjnwsqitqve`'s `down_revision` re-pointed `q8vd3mzk7w2p` → `3gufg5ykw01g` (commits `ef422dac`/`5191ab2f`); `51r81g9s8arp` skipped. **No cross-PR merge order.** qa + PROD DBs confirmed at `q8vd3mzk7w2p` (read-only). **Correction:** separate interview PRs [#1879](https://github.com/taller-projects/echo-backend/pull/1879)/[#1880](https://github.com/taller-projects/echo-backend/pull/1880) were opened first, then **closed + branches deleted** once folded in here. All 4 PR titles + descriptions updated. Dissolves the "future down_revision conflict" gotcha above.
